    ...
    def __init__(self,wafer_parameters):
        ...
        try:
            if len(wafer_parameters)>=13:
                ...
                self.thickness = 0
                self.set_wafer_size()
                self.xsize = wafer_parameters["xsize"]
                self.ysize = wafer_parameters["ysize"]
                # xmax & ymax optionals
                ...
                self.ymax = 0
                self.nchips = wafer_parameters["nchips"]
                self.nmodules = wafer_parameters["nmodules"]
                self.origin_chip = wafer_parameters["origin_chip"]
                self.home_chip = wafer_parameters["home_chip"]
                self.flat_orientation = wafer_parameters["flat_orientation"]
    ...
    def __call__(self):
        ...
        if status != "IDLE":
            ...
            if self.waferwindow.widgets.txtOriginChip.text()!="":
                ...
                die = self.waferwindow.wafer_parameters["wafer_positions"].index(real_coord)
                module = self.waferwindow.view_modules-1
